...
env = os.environ.copy()
env["COLUMNS"] = "240"
help_text = subprocess.check_output(["ytdown", "download", "--help"], text=True, env=env)
required = [
    "--quality", "--exact-quality", "--fallback", "--format-id", "--compat", "--container",
    "--output", "--output-dir", "-o", "--template", "--safe-names",
    "--file", "--playlist", "--playlist-items", "--archive", "--archive-path", "--rate-limit", "--retries",
    ...
    "--overwrite", "--sleep", "--max-sleep", "--socket-timeout", "--preset", "--config",
]
...
if [[ -z "$TEST_URL" ]]; then
  ...
  run_cmd "Live dry-run 720p nearest" ytdown download "$TEST_URL" --quality 720p --fallback nearest --compat --container mp4 --dry-run --output-dir "$REPORT_ROOT/live-dry-run-720p"
  run_cmd "Live dry-run 1080p nearest" ytdown download "$TEST_URL" --quality 1080p --fallback nearest --compat --container mp4 --dry-run --output-dir "$REPORT_ROOT/live-dry-run-1080p"
  run_cmd "Live dry-run 1440p nearest" ytdown download "$TEST_URL" --quality 1440p --fallback nearest --dry-run --output-dir "$REPORT_ROOT/live-dry-run-1440p"
  run_cmd "Live dry-run 2160p nearest" ytdown download "$TEST_URL" --quality 2160p --fallback nearest --dry-run --output-dir "$REPORT_ROOT/live-dry-run-2160p"
  run_cmd "Live dry-run metadata/subtitle flags" ytdown download "$TEST_URL" --quality 1080p --fallback nearest --subs --auto-subs --embed-subs --sub-langs "en.*" --thumbnail --write-info-json --write-description --dry-run --output-dir "$REPORT_ROOT/live-dry-run-metadata"
